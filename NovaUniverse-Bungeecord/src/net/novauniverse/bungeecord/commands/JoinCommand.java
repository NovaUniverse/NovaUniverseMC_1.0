...
public class JoinCommand extends Command {
	...
	public void execute(CommandSender sender, String[] args) {
		if (sender instanceof ProxiedPlayer) {
			if (args.length == 0) {
				sender.sendMessage(new TextComponent(ChatColor.RED + "Please provide a player to join"));
			} else {
						...
						if (isOnline) {
							...
							if (server != null) {
								if (rs.getBoolean("allow_join")) {
									...
									sender.sendMessage(new TextComponent(ChatColor.RED + username + " is on a server that does not allow the useage of the join command"));
								}
							} else {
